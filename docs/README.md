...
- Barber Directoryの福岡県理容所CSV取込は、既存の `barber_shops` を使い、重複する店舗テーブルは作らない。
- CSV取込は `/admin/barber-shops/import` の非公開管理画面で、CSV選択、プレビュー、必須項目チェック、重複候補確認、登録実行、結果確認の順に行う。
- CSVデータはmigrationやseedでは投入しない。PRマージ後、管理者が画面から手動で取込を実行する。
- 久留米市の掲載元が「福岡県理容生活衛生同業組合 久留米支部組合員名簿」の行は、久留米市の全理容所を保証する行政データとして扱わない。
- 店舗管理申請の審査は `/admin/barber-shops/claims` の非公開管理画面で行い、承認・却下はservice_role専用RPCをServer Actionから実行する。
