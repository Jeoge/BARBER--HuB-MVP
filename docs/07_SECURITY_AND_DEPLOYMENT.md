...
- `/partners`は未ログインでも送信できるが、ブラウザから`partner_inquiries`へ直接INSERTさせず、Server Actionで入力検証後にserver-onlyのservice role clientから保存する。ログイン中の送信だけ`user_id`を補助的に保存し、送信者向け一覧は作らない。
