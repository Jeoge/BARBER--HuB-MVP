...
4. 公式URL、必要に応じて都道府県・市区町村・掲載元名を入力し、`取得して確認` を押します。CSV、TSV、xlsx、HTML table、テキスト抽出可能なPDFに対応します。画像だけのPDFは手動確認します。
5. 自動判定された店名・住所・電話番号の列を確認し、必要なら列を選び直して `この列でCSVを作成` を押します。
6. 元データ件数、出力予定件数、空欄件数、除外行数、エラー件数と変換後の先頭行を確認します。重複判定は既存取込画面で行います。
7. `CSVをダウンロード` を押し、`既存のCSVインポート画面へ進む` を選びます。URL取得画面では既存の取込バッチやプレビューは作成しません。
