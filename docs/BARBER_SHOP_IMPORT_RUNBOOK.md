...
3. 公式ページや掲載ファイルURLから作成する場合は、`公式一覧からCSVを作成` を開きます。URL取得画面は `/admin/barber-shops/import/source` です。
4. 公式URLを入力し、`取得して確認` を押します。CSV、TSV、xlsx、HTML table、テキストPDFに対応します。画像だけのPDFは手動確認します。
5. `取得件数`、`有効件数`、空欄件数、`重複候補`、`エラー件数` と変換後の表を確認します。
6. `CSVをダウンロード` を押し、`既存のCSVインポート画面へ進む` を選びます。URL取得画面では既存の取込バッチやプレビューは作成しません。
7. `CSV選択` で手元のCSVを使う場合は、福岡県の統合済み店舗CSVを選びます。
8. ダウンロードしたCSVを `CSV選択` で選び、`内容をプレビュー` を押します。
9. `CSV行数`、`正常件数`、`エラー`、`完全一致`、`重複候補` を確認します。
10. `取込前確認` で次を確認します。
   - 都道府県別件数
   ...
   - 電話番号形式不正件数
   - 同一店名・同一住所候補件数
11. `必須項目・重複確認` でエラー行と重複候補を確認します。
12. 初回投入では、重複候補を十分に確認できない場合はチェックを入れずに `登録を実行` を押します。
13. 完了表示の `登録`、`スキップ`、`失敗` 件数を控えます。
14. `pnpm run barber-shops:verify` で公開検索の読み取り確認を行います。
