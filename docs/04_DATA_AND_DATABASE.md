...
- 有料記事は`articles.access_type`、`price_amount`、`currency`と、公開前本文を置かない`article_paid_sections`を分離する。本文セクションは投稿者または返金されていない購入済み本人だけがRLSでSELECTできる。
- Treatと有料記事購入は金額・手数料・宛先Connect account・Checkout Session・PaymentIntent・状態をそれぞれ`content_treats`と`paid_article_purchases`に記録する。クライアントからのINSERT・UPDATEは許可しない。
- `stripe_webhook_events`とservice role専用RPCでWebhookを冪等に処理し、`checkout.session.completed`の検証後だけ購入・Treatを完了にする。
