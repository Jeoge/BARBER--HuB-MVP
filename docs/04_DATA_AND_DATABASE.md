...
- `backroom_posts`: DELETEは`auth.uid() = user_id`かつBack Room参加中の本人だけに許可する。削除Server Actionも`id`、`user_id`、`is_deleted = false`で所有確認し、DELETE時に`id`と`user_id`を再指定する。管理者allowlistだけでは削除できない。
...
- `backroom-images`: private bucket。Storage objectの直接SELECT policyは作らず、親投稿・コメントの本人に限定したupload / update / delete policyをpath検証付きで持つ。正式なスレッド削除では通常の認証済みclientとowner-only RLSでDB親行を先に削除し、事前取得・所属検証済みのobject pathだけをservice roleのserver-only cleanupへ渡す。Storage削除失敗ではDB削除を巻き戻さない。
...
## Treat・有料記事・Back Roomコメントいいね

- `articles.category`を既存の主カテゴリとして残し、複数カテゴリは`article_category_assignments`で追加管理する。経験記事は主カテゴリ「経験記事」に加えて複数の関連カテゴリを選べる。
- 有料記事は`articles.access_type`、`price_amount`、`currency`と、公開前本文を置かない`article_paid_sections`を分離する。本文セクションは投稿者または`completed`・`partially_refunded`の購入済み本人だけがRLSでSELECTでき、`refunded`（全額返金）だけが閲覧権限を失う。
- Treatと有料記事購入は金額・手数料・宛先Connect account・Checkout Session・PaymentIntent・状態・返金額をそれぞれ`content_treats`と`paid_article_purchases`に記録する。クライアントからのINSERT・UPDATEは許可しない。購入は同一利用者・同一記事の有効なpending Checkoutを再利用し、期限切れ・利用不能なpendingは状態を更新してから再作成する。
- `stripe_webhook_events`とservice role専用RPCでWebhookを冪等に処理し、`checkout.session.completed`の検証後だけ購入・Treatを完了にする。`charge.refunded`は元決済額と`amount_refunded`を照合し、`partially_refunded`と`refunded`を区別する。
- Back Roomコメントのいいねは`backroom_comment_likes`の複合主キーで重複を防ぎ、RLSとServer Actionの両方で本人コメントへのいいねを拒否する。公開数は集計RPCで返し、誰が押したかは公開しない。
...
- プロフィール画像を削除すると、対応する`profiles.avatar_url`または`profiles.cover_url`を`null`で保存する。既存のカラム構造と`profile-images` bucketは維持し、新規migrationは追加しない。
- DB保存が成功した後にだけ、設定済みSupabaseのpublic object URL、bucket名`profile-images`、安全にdecodeできるobject path、本人の`${user.id}/`配下であることをすべて確認したStorage objectを削除する。
- 外部URL、別bucket、解析不能なURL、別userのpathはStorageから削除しない。保存後の`avatar_url`と`cover_url`のどちらからも参照されなくなったobjectだけをcleanup対象にする。
