...
公式一覧URLからの補助変換は、管理者専用の `/admin/barber-shops/import/source` から既存の `barber_shop_import_batches` / `barber_shop_import_rows` を再利用する。公式ページの取得結果を一時ファイルや新規履歴テーブルへ保存せず、現在の7列CSVへ変換して既存プレビューへ渡す。取得画面では店舗登録RPCを実行せず、最終登録は既存の管理者操作に限定する。外部URLはHTTPS、DNS解決先の内部IP拒否、リダイレクト3回以内、15秒タイムアウト、8MB上限、MIME / 拡張子確認を行い、HTMLのscriptは実行しない。
