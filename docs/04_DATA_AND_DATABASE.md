...
- `verification_status` は `unverified`, `unclaimed`, `pending`, `verified`, `rejected`, `suspended` を使う。CSV取込の「未認証」は `unverified` として保存し、既存の `unclaimed` は互換値として残す。
...
- `list_public_news` は内部の `content_pillar` を使ってWORK/STYLE/TALKの偏りを抑えるが、分類値自体は一般ユーザーへ返さない。
