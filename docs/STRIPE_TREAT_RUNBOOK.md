...
- 二重決済、閲覧不可、法令上の返金は、Stripe Dashboardの対象決済とDBの`stripe_payment_intent_id`を照合して対応する。Webhookの再送は`stripe_webhook_events`で冪等に処理する。返金は元決済額とStripeの`amount_refunded`を照合し、一部返金は`partially_refunded`、全額返金は`refunded`にする。有料本文の閲覧権限を停止するのは全額返金だけである。
