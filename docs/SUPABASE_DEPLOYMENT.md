...
```text
2026-07-10 Database > Backups confirmed
```

この値が空の場合、本番migration workflowは止まります。
...
2. `Database`を開きます。
3. `Backups`を開きます。
4. 最新backupまたはPITRの利用可否を確認します。
5. 確認した日付と方法をGitHub Environment Variableへ登録します。
...
6. `Environment variables`へ`SUPABASE_BACKUP_CONFIRMED_AT`を登録します。
