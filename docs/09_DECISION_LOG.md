...
## Back Roomのスレッドとコメントへ画像を最大1枚添付する

- 日付: 2026年7月20日
- 状態: 決定・実装
- 決定: Back Roomのスレッドとコメントは、それぞれ画像を最大1枚添付できる。スレッドは既存のタイトル・本文必須条件を維持し、コメントは本文または画像のどちらかを必須とするため画像だけでも投稿できる。
- 理由: 営業後の相談や経験共有に必要な視覚情報を追加しながら、Snap、記事、トップ画面の既存導線とBack Room一覧のカード構造を変更しないため。
- 守ること: JPEG / PNG / WebPをブラウザで長辺1600px以内・EXIF除去・圧縮後1枚2MB以内へ圧縮し、HEIC / HEIFはブラウザ変換に成功した場合だけ受け付ける。`backroom-images` private bucketへ保存し、DBにはobject pathだけを保存、表示時だけ30分signed URLを発行する。投稿失敗時は、成功投稿として公開されていない作成途中の行とStorage objectを補償削除する。現在の正式な削除UI・削除Server Actionは追加せず、将来の正式削除ではDB側の削除またはsoft deleteを成功させた後にStorageを削除する。画像がない既存投稿との互換性を保つ。
- 一覧表示: Back Room一覧はカード高さと情報量を維持するため代表画像を表示せず、スレッド詳細で本文幅いっぱいに表示する。コメント画像はコメント本文下にカード幅いっぱいで表示する。
- 影響範囲: `backroom_thread_images`, `backroom_comment_images`, `backroom-images`, Back Room投稿・コメントフォーム、スレッド詳細、投稿失敗時の補償処理、RLS、signed URL発行。正式な削除機能は対象外。
- 関連PR・文書: [02_UI_AND_DESIGN.md](02_UI_AND_DESIGN.md), [03_PRODUCT_AND_FEATURES.md](03_PRODUCT_AND_FEATURES.md), [04_DATA_AND_DATABASE.md](04_DATA_AND_DATABASE.md), [07_SECURITY_AND_DEPLOYMENT.md](07_SECURITY_AND_DEPLOYMENT.md)

## Back Roomの画像だけコメントはDB側で原子的に確定する

- 日付: 2026年7月20日
- 状態: 決定・実装
- 決定: `backroom_comments.body` を画像だけコメントのためnullableにする一方、通常authenticatedのINSERT / UPDATE RLSでは公開状態の本文を非空に維持する。画像だけコメントはStorage upload成功後、`create_backroom_image_comment` SECURITY DEFINER RPCがcomment path、親スレッド、参加権限、画像メタデータ、Storage objectを検証し、コメント行と画像行を同一トランザクションで作成する。さらに遅延constraint triggerで本文または画像行の存在をDB確定時に強制する。
- 理由: Server Actionの入力チェックだけでは、Supabase APIへ直接INSERT / UPDATEされたbody=nullや空文字のコメントを防げないため。Storage uploadや画像行保存の途中に公開SELECTされるコメント行も作らず、失敗時はStorage objectだけが一時的に残る可能性を許容して、公開本文から画像が消失する状態を避ける。
- 守ること: ダミー文字、空白、ゼロ幅文字で本文必須を回避しない。RPC実行や補償削除にservice role keyをクライアントへ渡さない。通常のBack Room閲覧・投稿参加条件、既存の削除UIなし方針、画像なし既存投稿との互換性を維持する。
- 影響範囲: `202607200002_backroom_comment_integrity.sql`, `app/backroom/actions.ts`, server-only画像upload / cleanup、コメントRLS、コメント画像の取得表示。
- 関連PR・文書: [03_PRODUCT_AND_FEATURES.md](03_PRODUCT_AND_FEATURES.md), [04_DATA_AND_DATABASE.md](04_DATA_AND_DATABASE.md), [07_SECURITY_AND_DEPLOYMENT.md](07_SECURITY_AND_DEPLOYMENT.md)

## 公式一覧URLはCSVダウンロード経由で既存取込へ接続する

- 日付: 2026年7月20日
- 状態: 決定・実装
- 決定: 自治体・保健所の公式ページまたは掲載ファイルURLから、CSV、TSV、xlsx、HTML table、テキスト抽出可能なPDFの一覧を現在の7列店舗CSVへ変換する補助モードを追加する。元列の自動判定を初期値とし、店名・住所・電話番号は管理者が列選択で補正できる。都道府県、市区町村、掲載元名も管理者入力で補完する。変換結果は管理者がCSVとしてダウンロードし、`/admin/barber-shops/import` で改めて選択する。既存の `barber_shop_import_batches` / `barber_shop_import_rows`、プレビュー、重複判定、登録処理は既存画面だけで実行する。新規テーブル、Cron、OCR、自動登録は追加しない。
- 理由: URL検索、コピー、ChatGPTへの貼り付けを減らしながら、既存CSV取込処理を変更・複製せず、既存の重複・未認証・手動登録フローとデータ構造を維持するため。
- 守ること: 代表者氏名など不要な個人情報は出力しない。住所から自治体を推測して確定しない。CSVはUTF-8 BOMで出力し、式として解釈されるセルを安全化する。HTTPS、内部IP拒否、タイムアウト、取得容量、リダイレクト回数、MIME確認を行う。画像だけまたは複雑なPDF、OCR、CAPTCHA、ログイン、JavaScript操作が必要な一覧は自動解析せず、手動確認へ案内する。
- 追加判断: HTMLの検索フォームだけのページには検索操作を行わず、検索結果URLまたはファイルURLを案内する。ページネーションは1ページ分だけ取得し、ページ内表示件数とページネーション検出を管理者へ明示する。組合加盟店一覧は掲載元名で区別し、公的な全施設一覧と同一視しない。
- 影響範囲: `/admin/barber-shops/import/source`, `lib/barber-import/source.ts`, 既存CSVプレビュー導線。
- 関連PR・文書: [03_PRODUCT_AND_FEATURES.md](03_PRODUCT_AND_FEATURES.md), [04_DATA_AND_DATABASE.md](04_DATA_AND_DATABASE.md), [07_SECURITY_AND_DEPLOYMENT.md](07_SECURITY_AND_DEPLOYMENT.md)

## Back Roomのスレッドは作成者本人だけが詳細画面から物理削除する

- 日付: 2026年7月22日
- 状態: 決定・実装
- 決定: Back Roomの実データスレッドは、詳細画面で確認した後、`backroom_posts.user_id = auth.uid()`を満たすスレッド作成者本人だけが物理削除できる。UIの本人限定表示に加え、Server Actionの所有再確認と既存owner-only DELETE RLSの両方で権限を保証する。管理者allowlistだけでは削除できない。
- 理由: 投稿者が自分のスレッドを管理できるようにしつつ、他人のスレッドIDへの書き換え、未ログイン操作、削除済み・存在しないIDによる削除を防ぎ、コメントや画像メタデータを残さないため。
- 守ること: DBでは通常の認証済みclientで親スレッドを先に削除し、コメント・スレッド画像メタデータ・コメント画像メタデータをcascade削除する。事前に全画像pathを取得して親ID所属を検証し、DB削除成功後にだけprivate Storage objectをserver-onlyで削除する。Storage削除失敗ではDBを巻き戻さず、画面には成功を返して詳細をサーバーログへ残す。記事とSnapの既存「削除依頼」は変更しない。
- 影響範囲: `/backroom/[id]`, `/backroom`, `app/backroom/actions.ts`, `backroom_posts`, `backroom_comments`, `backroom_thread_images`, `backroom_comment_images`, private `backroom-images`。既存RLSと外部キーcascadeを使うため新規migrationは追加しない。
- 関連PR・文書: [02_UI_AND_DESIGN.md](02_UI_AND_DESIGN.md), [03_PRODUCT_AND_FEATURES.md](03_PRODUCT_AND_FEATURES.md), [04_DATA_AND_DATABASE.md](04_DATA_AND_DATABASE.md), [07_SECURITY_AND_DEPLOYMENT.md](07_SECURITY_AND_DEPLOYMENT.md)

## Back Room削除は画像テーブル未適用時も画像なしスレッドを止めない

- 日付: 2026年7月22日
- 状態: 決定・実装
- 決定: 作成者本人によるBack Roomスレッド削除では、画像metadata取得がPostgreSQL `42P01`またはPostgREST `PGRST205`で失敗し、かつエラー内の対象名が`backroom_thread_images`または`backroom_comment_images`と正確に一致する場合だけ、その画像pathを空配列としてDB削除へ進む。コメント0件では`backroom_comment_images`を問い合わせない。
- 理由: 画像migration未適用またはschema cache未更新の環境でも、画像metadata自体が存在しない画像なしスレッドの本人削除を、任意機能の事前取得だけで止めないため。
- 守ること: 通信、認証、RLS、timeout、code不一致、別テーブル、不正Storage path等は従来どおり削除を中止する。本人所有の二重確認とowner-only DELETE RLSを維持し、DB親行を`id`、`user_id`、`is_deleted = false`でDELETEしてexact count 1件を確認した後にだけStorageを削除する。Storage削除失敗ではDB削除を巻き戻さない。新規migrationは追加せず、記事とSnapの削除フローを変更しない。
- 影響範囲: `app/backroom/actions.ts`, Back Room画像metadata取得、削除成功判定、関連検証test。
- 関連PR・文書: [03_PRODUCT_AND_FEATURES.md](03_PRODUCT_AND_FEATURES.md), [04_DATA_AND_DATABASE.md](04_DATA_AND_DATABASE.md), [07_SECURITY_AND_DEPLOYMENT.md](07_SECURITY_AND_DEPLOYMENT.md)

## PARTNERSカードと協賛・広告掲載問い合わせを追加する

- 日付: 2026年7月21日
- 状態: 決定・実装
- 決定: トップページ全コンテンツの最下部、Legal Links直前にPARTNERSカードを配置し、`/partners/contact`で協賛・広告掲載・タイアップ・共同企画・その他の相談を受け付ける。業種一覧や具体的な料金表は掲載せず、内容と費用は個別に検討する。既存の`/partners`はメーカー・ディーラー情報等のページとして維持する。
- 決定: 問い合わせは未ログインでも送信できる専用フォームとし、`partner_inquiries`へ非公開保存する。一般ユーザーへの直接INSERT / SELECT / UPDATE / DELETE権限は与えず、検証済みServer Actionと既存管理者allowlistによる`/admin/partner-inquiries`だけで扱う。
- 決定: honeypot、Server Action・DB双方の入力検証、同一メールアドレスの短時間連続送信制限、送信完了状態へのリダイレクトでスパム・二重送信を抑止する。IP平文保存、外部CAPTCHA、メール通知サービスは追加しない。
- 理由: 既存トップの世界観と広告表示機能を変えず、企業・団体向けの正式窓口と、個人情報を含む問い合わせの非公開運用を分離するため。
- 影響範囲: `/`, `/partners`, `/partners/contact`, `/admin/partner-inquiries`, `partner_inquiries`, `content_ads`との役割整理、関連UI・DB・セキュリティdocs。`source_page`は`/partners/contact`に固定する。
- 関連PR・文書: [02_UI_AND_DESIGN.md](02_UI_AND_DESIGN.md), [03_PRODUCT_AND_FEATURES.md](03_PRODUCT_AND_FEATURES.md), [04_DATA_AND_DATABASE.md](04_DATA_AND_DATABASE.md), [05_OPERATIONS_AND_GOVERNANCE.md](05_OPERATIONS_AND_GOVERNANCE.md), [07_SECURITY_AND_DEPLOYMENT.md](07_SECURITY_AND_DEPLOYMENT.md)

## プロフィール画像はカバーより前面、削除は保存確定型にする

- 日付: 2026年7月23日
- 状態: 決定・実装
- 決定: 公開プロフィールの丸いアイコンはカバー写真へ重ねたまま、stacking orderでカバーより前面に表示する。本人、他ユーザー、未ログイン閲覧者で共通の表示とする。写真削除は即時削除せず保存時に確定し、DB更新後に安全確認済みのStorage objectだけをcleanupする。
- 理由: 既存のプロフィールカードの位置・サイズ・比率を変えずに重なり順を直し、保存前の誤削除と他人・外部objectの誤削除を防ぐため。
- 守ること: `avatar_url` / `cover_url`をDBで`null`にする前に旧objectを削除しない。保存失敗時は新規objectを補償削除し、旧object削除失敗ではDB変更を巻き戻さない。OSが表示するファイルプロバイダーやGoogleドライブ等のメニューはWeb側から個別制御しない。
- 影響範囲: `/profiles/[id]`、`/mypage/profile/edit`、プロフィール画像Server Action、`profile-images`の既存Storage policy。DB schema、RLS、migrationは変更しない。
- 関連PR・文書: [02_UI_AND_DESIGN.md](02_UI_AND_DESIGN.md), [03_PRODUCT_AND_FEATURES.md](03_PRODUCT_AND_FEATURES.md), [04_DATA_AND_DATABASE.md](04_DATA_AND_DATABASE.md), [07_SECURITY_AND_DEPLOYMENT.md](07_SECURITY_AND_DEPLOYMENT.md)

## フォロー通知と本人用フォロワー一覧を追加する

- 日付: 2026年7月23日
- 状態: 決定・実装
- 決定: フォロー成立時だけDB triggerからfollow通知を作成し、フォロー解除時に対応する既読・未読通知を削除する。過去フォローのbackfillは行わない。
- 決定: 通知のactor画像・表示名はactorプロフィールへ移動し、通知本文は既存のSnap / 記事 / コメント導線を維持する。フォロー通知本文はactorプロフィールへ移動する。
- 決定: フォロワー一覧はログイン本人のマイページにだけ追加し、公開プロフィールには人物一覧を表示しない。
- 決定: 公開プロフィールへThanks総数、いいね総数、Thanksポイント、本人専用反応統計は追加しない。public reaction countの方針は変更しない。
- 理由: 通知の「誰が」と「何に対して」を分離しながら既存のカード見た目と投稿導線を守り、フォロー関係の確認範囲を本人の管理画面に限定するため。
- 影響範囲: `follows`、`notifications`、通知RPC / RLS、通知一覧、マイページ、関連UI / DB / security docs。
- 関連文書: [02_UI_AND_DESIGN.md](02_UI_AND_DESIGN.md), [03_PRODUCT_AND_FEATURES.md](03_PRODUCT_AND_FEATURES.md), [04_DATA_AND_DATABASE.md](04_DATA_AND_DATABASE.md), [07_SECURITY_AND_DEPLOYMENT.md](07_SECURITY_AND_DEPLOYMENT.md)

## Back Room画像はサムネイル表示、削除はマイページ起点にする

- 日付: 2026年7月23日
- 状態: 決定・実装
- 決定: Back Roomスレッド本文とコメントの画像は、通常画面では320px変換URLを使った最大160px前後のサムネイルで表示し、タップ時だけ変換なしの短時間signed URLをビューアへ読み込む。画像全体を縦横比を保って確認できるようにし、過去画像の一括変換・再アップロードは行わない。
- 決定: 新規選択画像は1枚10MB以下、圧縮後は長辺1600px以内・1枚2MB以内とする。既存のServer Action、MIME、シグネチャ、寸法、Storage path検証は維持する。
- 決定: スレッド詳細から削除ボタンを撤去し、本人のマイページ「自分のBack Roomスレッド」からだけSnapと同様の確認画面を経て削除を開始する。削除処理はServer ActionでAuth本人と`backroom_posts.user_id`を照合し、既存のowner-only RLS、関連行のcascade、検証済みStorage cleanupを維持する。
- 理由: 本文やコメントの閲覧を大きな画像で妨げず、削除操作を本人の管理画面に集約して誤操作と他人のスレッド削除を防ぐため。
- 影響範囲: Back Room詳細・コメント画像表示、画像signed URL生成、画像選択上限、マイページ、Back Roomスレッド削除Server Action。DB schema、migration、Storage policy、RLSは変更しない。
- 関連文書: [02_UI_AND_DESIGN.md](02_UI_AND_DESIGN.md), [03_PRODUCT_AND_FEATURES.md](03_PRODUCT_AND_FEATURES.md), [04_DATA_AND_DATABASE.md](04_DATA_AND_DATABASE.md), [07_SECURITY_AND_DEPLOYMENT.md](07_SECURITY_AND_DEPLOYMENT.md)

## Back Room画像変換は任意機能として保存済み圧縮画像へfallbackする

- 日付: 2026年7月23日
- 状態: 決定・実装
- 決定: Back Roomの通常表示はStorage Image Transformationsによる320pxサムネイルを優先するが、変換URLが空・不正・発行不可、またはブラウザでの読み込みに失敗した場合は、同じserver-only signed URL発行とRLS / private Storageの権限を保った保存済み圧縮画像URLへ1回だけfallbackする。両方の読み込みに失敗した場合も、本文・コメント・ページ全体を壊さず該当画像内のエラー表示で終える。
- 守ること: ビューアはタップ時だけ長辺1600px以内・2MB以内の保存済み圧縮画像を読み込み、元の10MB画像を保存・表示しない。スレッドとコメントへ同じルールを適用し、fallbackループ、任意外部URL、service role keyのクライアント露出、画像テーブルやmigrationの一括変更を行わない。
- 理由: Supabase Image Transformationsが利用できない環境や履歴画像でも、画像が消えず、通常時の通信量と既存の画像権限を両立させるため。
- 影響範囲: Back Room画像signed URL生成、スレッド・コメント詳細の画像表示、関連UI / product / data / security docs。DB schema、migration、Snap、削除導線は変更しない。
- 関連PR・文書: [02_UI_AND_DESIGN.md](02_UI_AND_DESIGN.md), [03_PRODUCT_AND_FEATURES.md](03_PRODUCT_AND_FEATURES.md), [04_DATA_AND_DATABASE.md](04_DATA_AND_DATABASE.md), [07_SECURITY_AND_DEPLOYMENT.md](07_SECURITY_AND_DEPLOYMENT.md)

## Treat・有料記事はStripe Connect destination chargeとWebhook確定にする

- 日付: 2026年7月24日
- 状態: 決定・実装（Production有効化は保留）
- 決定: TreatはSnap、閲覧可能な記事、Back Roomスレッド・コメントへ送れる任意の感謝とし、有料記事は経験記事・講習会レポートに限定する。既存の`articles.category`と既存リアクションデータは残し、複数カテゴリは中間テーブルを追加する。
- 決定: 決済はStripe Checkoutのdestination chargeを使い、販売額の15%をプラットフォーム手数料、残額を投稿者のConnect account宛とする。購入・Treatの確定はredirectではなく署名検証済みWebhookで行い、イベントIDと決済レコードで冪等に処理する。
- 決定: 有料本文は公開部分と別テーブルに分離し、投稿者または全額返金されていない購入済み本人だけにRLSで返す。`BARBER_HUB_MONETIZATION_ENABLED`がOFFの間はSnap・記事の既存いいねUIを維持し、ONにしたときだけTreatへ切り替える。Back Roomコメントのいいねは公開件数だけを表示する。返金は元決済額と`amount_refunded`を照合し、一部返金と全額返金を区別する。
- 理由: 既存コンテンツと投稿体験を保ちつつ、決済完了の整合性、投稿者への配分、購入前の本文非公開をDBとWebhookで担保するため。
- 影響範囲: Stripe環境変数、Webhook、Connect受取設定、Treat・有料記事UI、Back Roomコメントいいね、通知、特商法表示、migration、RLS、運用Runbook。
- 関連文書: [04_DATA_AND_DATABASE.md](04_DATA_AND_DATABASE.md), [06_MONETIZATION_AND_PARTNERS.md](06_MONETIZATION_AND_PARTNERS.md), [07_SECURITY_AND_DEPLOYMENT.md](07_SECURITY_AND_DEPLOYMENT.md), [STRIPE_TREAT_RUNBOOK.md](STRIPE_TREAT_RUNBOOK.md)
