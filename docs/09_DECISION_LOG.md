...
- 守ること: 確認リンクが使用済み、期限切れ、二重クリック、またはパラメータ不足で処理できない場合も、エラー表示前に既存の有効セッションを確認する。正常なセッションがある場合はメールアドレス・パスワードを再入力させない。`next` は相対パスだけ許可する。認証コード、token、メールアドレス、パスワード、Supabase詳細エラーはログや画面へ出さない。
