...
- 日付: 2026年7月時点
- 状態: 決定
- 決定: 記事画像は1記事1枚までとし、ブラウザ圧縮後のJPEG / WebPを `article-images` bucketへ保存し、`articles.image_url` / `articles.image_path` を使って表示する。
- 理由: 既存DBカラムを活用し、原寸画像や危険なファイル名を保存せず、記事詳細・一覧・トップ掲載で同じ画像参照を使えるようにするため。
