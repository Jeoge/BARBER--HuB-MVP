...
## 公式一覧URLから既存CSVプレビューへ渡す

- 日付: 2026年7月20日
- 状態: 決定・実装
- 決定: 自治体・保健所の公式ページまたは掲載ファイルURLから、CSV、TSV、xlsx、HTML table、テキストPDFの一覧を現在の7列店舗CSVへ変換する補助モードを追加する。変換結果は既存の `barber_shop_import_batches` / `barber_shop_import_rows` を使ってプレビューし、店舗登録は既存の管理者操作でのみ行う。新規テーブル、Cron、OCR、自動登録は追加しない。
- 理由: URL検索、コピー、ChatGPTへの貼り付けを減らしながら、既存の重複・未認証・手動登録フローとデータ構造を維持するため。
