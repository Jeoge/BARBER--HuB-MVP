...
- 守ること: JPEG / PNG / WebPをブラウザで長辺1600px以内・EXIF除去・圧縮後1枚2MB以内へ圧縮し、HEIC / HEIFはブラウザ変換に成功した場合だけ受け付ける。`backroom-images` private bucketへ保存し、DBにはobject pathだけを保存、表示時だけ30分signed URLを発行する。投稿失敗時は補償削除し、スレッド・コメント削除時は関連Storage objectと画像レコードも削除する。画像がない既存投稿との互換性を保つ。
- 一覧表示: Back Room一覧はカード高さと情報量を維持するため代表画像を表示せず、スレッド詳細で本文幅いっぱいに表示する。コメント画像はコメント本文下にカード幅いっぱいで表示する。
- 影響範囲: `backroom_thread_images`, `backroom_comment_images`, `backroom-images`, Back Room投稿・コメントフォーム、スレッド詳細、削除処理、RLS、signed URL発行。
