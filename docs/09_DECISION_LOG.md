...
- 日付: 2026年7月19日
- 状態: 決定
- 決定: 経営、集客、AI、技術、道具の各メニューは投稿カテゴリーから自動表示し、固定記事、固定関連記事、関連Snap、固定広告を本番fallbackにしない。記事0件時は短い空状態、新着記事導線、ほかのテーマチップだけで成立させる。
...
- 守ること: PR、広告、Sponsoredのいずれかを明示する。広告がない場合は空枠や広告募集中カードを出さない。anon / authenticatedは有効な広告だけSELECTでき、広告管理はservice roleを使う管理者用サーバー処理に限定する。
