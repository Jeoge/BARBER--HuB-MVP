...
- 日付: 2026年7月20日
- 状態: 決定・実装
- 決定: 自治体・保健所の公式ページまたは掲載ファイルURLから、CSV、TSV、xlsx、HTML table、テキストPDFの一覧を現在の7列店舗CSVへ変換する補助モードを追加する。変換結果は管理者がCSVとしてダウンロードし、`/admin/barber-shops/import` で改めて選択する。既存の `barber_shop_import_batches` / `barber_shop_import_rows`、プレビュー、重複判定、登録処理は既存画面だけで実行する。新規テーブル、Cron、OCR、自動登録は追加しない。
- 理由: URL検索、コピー、ChatGPTへの貼り付けを減らしながら、既存CSV取込処理を変更・複製せず、既存の重複・未認証・手動登録フローとデータ構造を維持するため。
- 守ること: 代表者氏名など不要な個人情報は出力しない。HTTPS、内部IP拒否、タイムアウト、取得容量、リダイレクト回数、MIME確認を行う。画像だけまたは複雑なPDFは自動登録せず、手動確認へ案内する。
