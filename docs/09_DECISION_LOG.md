...
- 日付: 2026年7月18日
- 状態: 決定
- 決定: CSV列「認証状態」の「未認証」は `verification_status = 'unverified'` として保存する。既存の `unclaimed` は互換値として残し、公開UIではどちらも「未認証」と表示する。
- 理由: CSV由来の未認証店舗を、オーナー未申請状態として明確に扱い、認証済み店舗と誤認させないため。
- 守ること: 認証済みバッジは `verified` の店舗だけに表示する。CSV取込時の `owner_user_id` はNULLにする。一般ユーザーが `verification_status` や管理項目を直接変更できないよう、RLSと列権限を確認する。
