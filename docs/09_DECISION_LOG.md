...
## 記事画像はarticle-images bucketと既存articles画像カラムを使う

- 日付: 2026年7月時点
- 状態: 決定
- 決定: 記事画像は1記事1枚までとし、ブラウザ圧縮後のJPEG / WebPをprivateな `article-images` bucketへ保存する。新規記事では `articles.image_path` を保存し、表示時に公開中・未削除記事だけサーバー側で30分程度の短時間signed URLを発行する。`image_path` がない既存記事は `articles.image_url` を互換表示に使える。
- 理由: 既存DBカラムを活用しつつ、非公開化・論理削除後に恒久公開URLで画像が残り続けるリスクを避けるため。
- 影響範囲: 記事投稿、Storage RLS、記事詳細、カテゴリー別一覧、マイページ、投稿者プロフィール、EDITOR'S PICK。
